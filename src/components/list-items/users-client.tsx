...
export function UsersClient() {
  ...
  const colDefs: ColDef<SigmaUserDto>[] = useMemo(
    () => [
      ...
      { field: "jobTitle", headerName: "Job Title", sortable: true, filter: "agTextColumnFilter", width: 200 },
      {
    ...
    <>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage and audit user directory identities.</p>
        </div>
        <div className="flex items-center gap-3">
          {!isLoading && <span className="text-sm text-muted-foreground">{filteredUsers.length} users</span>}
